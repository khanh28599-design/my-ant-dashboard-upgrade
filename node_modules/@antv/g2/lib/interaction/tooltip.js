...
function createTooltip(container, x0, y0, position, enterable, bounding, containerOffset, css = {}, offset = [10, 10]) {
    const defaults = {
        [(0, constant_1.g2Selector)('tooltip')]: {},
        [(0, constant_1.g2Selector)('tooltip-title')]: {
            overflow: 'hidden',
            'white-space': 'nowrap',
    ...
    const tooltipElement = new component_1.Tooltip({
        ...
        style: {
            ...
            offset,
            template: {
                prefixCls: constant_1.G2_CLASS_PREFIX,
            },
...
function showTooltip({ root, data, x, y, render, event, single, position = 'right-bottom', enterable = false, css, mount, bounding, offset, }) {
    ...
    tooltipElement.update(Object.assign({ x,
        y, data: items.map((item) => (Object.assign(Object.assign({}, item), { value: !item.value && item.value !== 0 ? '' : item.value }))), title,
        position,
...
function heatmapItem(element) {
    ...
    const hasCustomItems = items.some((item) => transform_1.CALLBACK_ITEM_SYMBOL in item);
    if (hasCustomItems) {
...
function updateMarker(root, { data, style, theme }) {
    ...
    const markers = data
        ...
        const shape = new g_1.Circle({
            className: `${constant_1.G2_CLASS_PREFIX}tooltip-marker`,
...
function equalBandWidth(scale) {
    const { x } = scale;
    if (!x || !x.valueBandWidth)
        return true;
    const { valueBandWidth } = x;
    if ((0, util_1.isNumber)(valueBandWidth))
        return true;
...
function findSingleElement({ root, event, elements = [], coordinate, scale, shared, }) {
    ...
    const xof = scaleSeries && scaleSeries.valueBandWidth
        ? (d) => {
    ...
    const element = isBar
        ? (event) => {
            ...
            if (!shared) {
                ...
                if (isGrouped)
                    return findElementByTarget(event) || isGrouped;
            }
            return target;
        }
        : findElementByTarget;
...
function seriesTooltip(root, _a) {
    var { elements: elementsof, sort: sortFunction, filter: filterFunction, scale, coordinate, crosshairs, crosshairsX, crosshairsY, render, groupName, emitter, wait = 50, leading = true, trailing = false, startX = 0, startY = 0, body = true, single = true, position, enterable, mount, bounding, theme, offset, disableNative = false, marker = true, preserve = false, style: _style = {}, css = {}, clickLock = false, disableAutoHide = false } = _a, rest = __rest(_a, ["elements", "sort", "filter", "scale", "coordinate", "crosshairs", "crosshairsX", "crosshairsY", "render", "groupName", "emitter", "wait", "leading", "trailing", "startX", "startY", "body", "single", "position", "enterable", "mount", "bounding", "theme", "offset", "disableNative", "marker", "preserve", "style", "css", "clickLock", "disableAutoHide"]);
...
single = true, position, enterable, datum, view, mount, bounding, theme, offset, shared = false, body = true, disableNative = false, preserve = false, css = {}, clickLock = false, disableAutoHide = false, }) {
    const elements = elementsof(root);
...
function Tooltip(options) {
    ...
    return (target, viewInstances, emitter) => {
        ...
        if (isSeries && facet) {
            ...
            // @ts-ignore
            return seriesTooltip(root, Object.assign(Object.assign({}, rest), { theme, elements: () => elements, scale,
